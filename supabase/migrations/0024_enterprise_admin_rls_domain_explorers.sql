-- Enterprise org admins must see and claim same-domain users with org_id NULL.
-- Policies must NOT subquery public.users directly — that re-enters RLS and causes infinite recursion.
-- Use SECURITY DEFINER helpers (same pattern as current_user_org_id()).
