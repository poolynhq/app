-- Per-rider settlement amounts (detour-attributed shares from the client pricing model).
DROP FUNCTION IF EXISTS public.poolyn_crew_trip_finish_and_settle_credits(uuid, integer);

CREATE OR REPLACE FUNCTION public.poolyn_crew_trip_finish_and_settle_credits(
  p_trip_instance_id uuid,
  p_contribution_credits_by_rider jsonb
)
RETURNS json
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_contribution_credits_by_rider IS NULL OR jsonb_typeof(p_contribution_credits_by_rider) <> 'object' THEN
  ...
  LOOP
    v_contrib_txt := p_contribution_credits_by_rider ->> (v_rider_uid::text);
    IF v_contrib_txt IS NULL OR length(trim(v_contrib_txt)) = 0 THEN
      v_rider_contrib := 0;
    ELSE
      BEGIN
        v_rider_contrib := trim(v_contrib_txt)::integer;
      EXCEPTION WHEN invalid_text_representation THEN
        RETURN json_build_object('ok', false, 'reason', 'invalid_contribution');
      END;
    END IF;

    IF v_rider_contrib < 0 OR v_rider_contrib > 5000000 THEN
      RETURN json_build_object('ok', false, 'reason', 'invalid_contribution');
    END IF;

    IF v_rider_contrib = 0 THEN
      CONTINUE;
    END IF;

    v_fee := CASE
      WHEN public.is_user_org_member(v_rider_uid) THEN 0
      ELSE (ROUND(v_rider_contrib::numeric * 0.04))::integer
    END;
    v_debit := v_rider_contrib + v_fee;
    v_sum_contrib := v_sum_contrib + v_rider_contrib;
    ...
    v_rider_line := jsonb_build_object(
      ...
      'credits_contribution', v_rider_contrib,
  ...
  SET trip_finished_at = now(),
      ...
      settlement_summary = jsonb_build_object(
        ...
        'contribution_credits_by_rider', p_contribution_credits_by_rider,
...
REVOKE ALL ON FUNCTION public.poolyn_crew_trip_finish_and_settle_credits(uuid, jsonb) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.poolyn_crew_trip_finish_and_settle_credits(uuid, jsonb) TO authenticated;
